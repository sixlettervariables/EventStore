...
namespace EventStore.Core.DataStructures {
	public class LRUCache<TKey, TValue> : ILRUCache<TKey, TValue> {
		...
		public LRUCache(
			long capacity,
			Func<TKey, TValue, int> calculateSize = null) {
			Ensure.Nonnegative(capacity, nameof(capacity));
			_capacity = capacity;
			_size = 0L;
			_calculateItemSize = calculateSize ?? _unitSize;
		}
		...
		private void PutItem(TKey key, TValue value) {
			lock (_lock) {
				...
				EnsureCapacity(out _, out _);

				_items.Add(key, node);
				_orderList.AddLast(node);

				_onPut?.Invoke(node.Value.Value);
				_size += _calculateItemSize(key, value);
			}
		}
		...
		protected void EnsureCapacity(out int removedCount, out long removedSize) {
			lock (_lock) {
				var initialCount = _items.Count;
				var initialSize = _size;

				while (_items.Count > 0 && _size >= _capacity)
					RemoveFirstItem();
