...
namespace EventStore.Core.Caching {
	public class DynamicCacheManager:
		...
		private readonly TimeSpan MonitoringInterval = TimeSpan.FromSeconds(15);
		private readonly TimeSpan MinResizeInterval = TimeSpan.FromMinutes(10);
		...
		private void ResizeCachesIfNeeded() {
			...
			if (freeMem > keepFreeMem && DateTime.UtcNow - _lastResize < MinResizeInterval)
				return;
