		...
		public async Task can_cancel_during_calculation_and_resume() {
			...
			var (state, db) = await scenario
				...
				.AssertTrace(
					...
					Tracer.Line("    Commit"),
					Tracer.Line("    Begin"),
					Tracer.Line("        SetDiscardPoints(98, Discard before 2, Keep all)"),
		...
		public async Task can_cancel_during_chunk_execution_and_resume() {
			...
			var (state, db) = await scenario
				...
				.AssertTrace(
					...
					Tracer.Line("    Commit"),
					Tracer.Line("    Begin"),
					Tracer.Line("        SetDiscardPoints(ab-1, Discard before 1, Keep all)"),
		...
		public async Task can_cancel_during_index_execution_and_resume() {
			...
			var (state, db) = await scenario
				...
				.AssertTrace(
					...
					Tracer.Line("    Commit"),
					Tracer.Line("    Begin"),
					Tracer.Line("        SetDiscardPoints(98, Discard before 3, Keep all)"),
